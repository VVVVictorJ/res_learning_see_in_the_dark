...
checkpoint_dir = './old_version/Sony/'
result_dir= './old_version/'
...
def pack_raw(raw):
    ...
    im = np.maximum(im - 528, 0) / (4095 - 528)  # subtract the black level
...
ratio = [300,400,500,600,700,800,900,1000]

for i in range(len(ratio)-1):
    in_path = 'D:/test/iphone/7p/frame_2/IMG_6278.DNG'
    in_fn = os.path.basename(in_path)
    print(str(i)+'\t'+'start')
    start = time.time()
    raw = rawpy.imread(in_path)
    input_full = np.expand_dims(pack_raw(raw), axis=0) * ratio[i]
...
#     scale_full)  # scale the low-light image to the same mean of the groundtruth
    ...
    scipy.misc.toimage(output * 255, high=255, low=0, cmin=0, cmax=255).save(
        result_dir + 'final/10_r_%d_out.png' %  ratio[i])
